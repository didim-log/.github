<div align="center">
  
  <img src="../images/didimlog_logo.png" width="200" height="200"/>

  # DidimLog : 디딤로그

  **"이런 나라도 PS 알고리즘을 체계적으로 잘 풀 수 있지 않을까?"**
  
  <br>

  디딤로그는 백엔드 개발자 지망생이 **'골드' 난이도**까지 <br>
  좌절하지 않고 체계적으로 성장할 수 있도록 돕는 **학습 관리 & 회고 서비스**입니다.

  <br>

  <img src="https://img.shields.io/badge/Project-DidimLog-0078FF?style=flat-square&logo=github" />
  <img src="https://img.shields.io/badge/Status-In%20Development-green?style=flat-square" />
  <img src="https://img.shields.io/badge/Target-Gold%20Tier-FFD700?style=flat-square" />

</div>

<br>

## 🚀 Core Values (핵심 가치)

| 키워드 | 설명 |
| :---: | :--- |
| **Step-by-Step** | 사용자의 실력에 맞춘 **단계별 문제 추천**으로 '몰입' 경험을 제공합니다. |
| **Timer & Feedback** | 실전 감각을 위한 **타이머**와 오답/정답 피드백 시스템을 갖췄습니다. |
| **Log & Retrospect** | 단순 풀이를 넘어, **블로그 회고 가이드**를 통해 지식을 내재화합니다. |
| **Clean Code** | **클린 코드**의 엄격한 코드 품질과 컨벤션을 지향합니다. |

<br>

## 🛠 Tech Stack

**Backend**
<br>
<img src="https://img.shields.io/badge/Kotlin-7F52FF?style=for-the-badge&logo=kotlin&logoColor=white">
<img src="https://img.shields.io/badge/Spring%20Boot-6DB33F?style=for-the-badge&logo=springboot&logoColor=white">
<img src="https://img.shields.io/badge/JPA-59666C?style=for-the-badge&logo=hibernate&logoColor=white">
<img src="https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white">
<img src="https://img.shields.io/badge/Redis-DC382D?style=for-the-badge&logo=redis&logoColor=white">

**Frontend**
<br>
<img src="https://img.shields.io/badge/React-61DAFB?style=for-the-badge&logo=react&logoColor=black">
<img src="https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white">
<img src="https://img.shields.io/badge/Firebase-FFCA28?style=for-the-badge&logo=firebase&logoColor=black">

**Infra & Tools**
<br>
<img src="https://img.shields.io/badge/AWS%20EC2-FF9900?style=for-the-badge&logo=amazonec2&logoColor=white">
<img src="https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white">
<img src="https://img.shields.io/badge/GitHub%20Actions-2088FF?style=for-the-badge&logo=githubactions&logoColor=white">

<br>

## 🤝 Contribution & Guide

디딤로그는 코드 품질(Code Quality)을 가장 중요하게 생각합니다.
기여하시기 전에 아래 가이드를 꼭 확인해주세요.

* [📜 Git Commit Convention (AngularJS Style)](./DOCS/COMMIT_CONVENTION.md)
* [🧹 Code Style & PR Guide](./DOCS/PR_GUIDE.md)

<br>

<div align="center">
  Copyright © 2025 DidimLog Team. All rights reserved.
</div>
